...
const app = express()
const plugin = require('./function/handler')
...
class Request {
    constructor(req) {
        ...
        this.rawMessage = request.rawMessage || ""
        this.msgType = request.msgType || 0
        this.time = request.time || 0
    }
}
...
const middleware = async (req, res) => {
    ...
    if (req.method === "GET") {
        Promise.resolve(plugin.info(cb))
            .then(res => {
                fnContext.status(200)
        ...
        const fnRequest = new Request(req)
        Promise.resolve(plugin.handle(fnRequest, cb))
            .then(res => {
                fnContext.status(200)
                    .succeed(JSON.stringify(res))
            })
